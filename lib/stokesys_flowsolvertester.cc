...
void StokeSys::solveFlow()
{
    ...
    //======================================================================
    // Solve the equation
    KSPSetTolerances(ksp, 1.E-5, PETSC_DEFAULT, PETSC_DEFAULT, 60);
...
void ctx::calcRhs()
{
    ...
    if (ewald::phys::active) {
	    ...
	    for(int bidoof=0; bidoof < n; bidoof++)
	      {cout << bidoof << " v1 = (" << v(bidoof,0) << ", " << v(bidoof,1) << ", " << v(bidoof,2) << ")\n";}
        }
	...
	{ 
	    ...
	    VecSetValues(rhs_vec, 3*n, idx.data(), v.data(), ADD_VALUES);
	    for(int bidoof=0; bidoof < n; bidoof++)
              {cout << bidoof << " v2 = (" << v(bidoof,0) << ", " << v(bidoof,1) << ", " << v(bidoof,2) << ")\n";}
	...
	ewald::four::transform();
	    ...
	    VecSetValues(rhs_vec, 3*n, idx.data(), v.data(), ADD_VALUES);
	    for(int bidoof=0; bidoof < n; bidoof++)
              {cout << bidoof << " v3 = (" << v(bidoof,0) << ", " << v(bidoof,1) << ", " << v(bidoof,2) << ")\n";}
	...
	for(int bidoof=0; bidoof < n; bidoof++)
	  {cout << bidoof << " v4 = (" << v(bidoof,0) << ", " << v(bidoof,1) << ", " << v(bidoof,2) << ")\n";}
