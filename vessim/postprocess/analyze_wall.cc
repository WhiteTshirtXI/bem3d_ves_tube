...
string in_dir("D15sean/ht10/ca025/");
string out_dir("D15sean/anawall/");
...
int main(int argc, char **argv)
{
    ...
    string fn_restart = "D15sean/ht20/restart000000.dat";//param::getStringValue("RESTART");
    int lt_min = 3000;//param::getIntValue("LT_MIN");
    int lt_max = 40000;//param::getIntValue("LT_MAX");
    int lt_step = 50;//param::getIntValue("LT_STEP");

    if (param::exist("IN_DIR")) in_dir = "D15sean/ht10/ca05/"; //param::getStringValue("IN_DIR") + "/";
    if (param::exist("OUT_DIR")) out_dir = "D15sean/anawall/"; //param::getStringValue("OUT_DIR") + "/";
    ...
    for (int lt = lt_min; lt <= lt_max; lt += lt_step) 
    {
		...
		for (int iface = 0; iface < wall.numFaces(); iface++) 
		{
			...
			for (int l = 0; l < 3; ++l)  {
				...
				m_dadd(3, &wall.verts[ivert].f[0], f);
				}

			//cout << "Face " << iface << " f: " << f[0] << ' ' << f[1] << ' ' << f[2] << endl;
			//cout << "Area of face " << iface << ": " << face.area << endl;

			m_daxpy(3, THRD*face.area, f, &pgrad(nrec,0));
		}
		m_dscal(3, ewald::iVol, &pgrad(nrec,0));
