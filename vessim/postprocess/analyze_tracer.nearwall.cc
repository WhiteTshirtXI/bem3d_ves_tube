...
void readTracer()
{
    ...
    // Flip the tracer coordinates and velocities
    for (int irec = 0; irec < nrec; irec++) {
        for (int i = 0; i < ntrac; i++) {
	    if (xtrac(irec,i,2) > 0.5*ewald::L[2]) {
	        xtrac(irec,i,2) = ewald::L[2] - xtrac(irec,i,2);
		vtrac(irec,i,2) = -vtrac(irec,i,2);
	    }
	}
    }
...
void velProf()
{
    ...
    for (int irec = 0; irec < nrec; irec++)
    for (int i = 0; i < ntrac; i++) {
	int iz = (int)floor( (xtrac(irec,i,2) - zmin)*idz);
...
void correlation()
{
	...
	for (int i0 = 0; i0 < nrec; i0 += 10) {
	    ...
	    int iz = (int)nearbyint( (z0 - zmin)/dz );
