...
#include "miscUtils.h"
#include <time.h>
//With this version, you need to add i2 slices separately.
...
//    stks.readRestart("D/restart020000.dat");
	...
	//Initialize geometry information for rigids
	initialrigids = stks.numRigids();
    for (int irig = 0; irig < stks.numRigids(); irig++) {
        Rigid &rig = stks.rigids[irig];
	rig.updateGeometry();
    }
	...
	// Only add rigid at the specified line
	if ( i2 != atoi(argv[2])) occupied(i0,i1,i2) = true;
    ...
    printf("Current number of platelets = %d\n", initialrigids);
    printf("Number of available rigid sites = %d\n", nrigid_avail);

    int nrigid;
    printf("Number of additional platelets =? ");
    //cin >> nrigid;
    nrigid = atoi(argv[3]);
    nrigid = min(nrigid, nrigid_avail);

    stks.rigids.resize(nrigid+initialrigids);
    for (int irigid = 0; irigid < nrigid; irigid++) {
	Rigid &rigid = stks.rigids[initialrigids+irigid];
